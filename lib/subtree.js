...
module.exports = (function () {
    ...
    Subtree._cmd_pullall = function (args, callback) {
        var subtrees = require(path.join(process.cwd(), 'subtrees.json'));
        var i = 0;
        var subtreesLength = subtrees.length;
        var configs = [];
        for (var packageName in subtrees) {
            configs.push({config: subtrees[packageName], packageName: packageName});
        }
        function next() {
            if(i < configs.length) {
                Subtree.run(["pull", configs[i].packageName], function(err, result) {
                    if (err) {
    ...
    Subtree._cmd_pushall = function (args, callback) {
        ...
        function next() {
            if(i < configs.length) {
                var branch = customBranch ? customBranch : configs[i].branch;
                Subtree.run(["push", configs[i].packageName, branch], function(err, result) {
                    if (err) {
                        console.error(('ERROR: ' + err).red);
                        process.exit(1);
                    } else {
